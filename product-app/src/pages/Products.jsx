...
If the issue persists, please contact our support team for further assistance</h1>
  ...
  return (
    ...
    <div className='flex flex-wrap gap-4 content-center items-center m-10'>
    {data && data.products && data.products.map((item) => {
      return (
       <Card key={item.id} id={item.id} title={item.title} description={item.description} img={item.thumbnail}/>
      );
    })}
  </div>
);
